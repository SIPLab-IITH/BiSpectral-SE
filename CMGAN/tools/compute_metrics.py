...
def compute_metrics(cleanFile, enhancedFile, Fs, path):
    ...
    return pesq_mos, CSIG, CBAK, COVL, segSNR, STOI, sisnr

def si_snr(clean_speech, processed_speech):
    smallVal = np.finfo("float").eps

    dot = np.sum(processed_speech * clean_speech, axis=0, keepdims=True)
    s_target_energy = (np.sum(clean_speech ** 2, axis=0, keepdims=True) + smallVal)
    proj = dot * clean_speech / s_target_energy

    # e_noise = s' - s_target
    e_noise = processed_speech - proj

    # SI-SNR = 10 * log_10(||s_target||^2 / ||e_noise||^2)
    si_snr_beforelog = np.sum(proj ** 2, axis=0) / (np.sum(e_noise ** 2, axis=0) + smallVal)
    SI_SNR = 10.*np.log10(si_snr_beforelog + smallVal)    
    return SI_SNR
