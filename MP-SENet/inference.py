...
def inference(a):
    ...
    with torch.no_grad():
        for index in track(test_indexes):
            ...
            audio_g = mag_pha_istft(amp_g, noisy_pha, h.n_fft, h.hop_size, h.win_size, h.compress_factor)
            audio_g = audio_g / norm_factor

            output_file = os.path.join(a.output_dir, index)

            #sf.write(output_file, audio_g.squeeze().cpu().numpy(), h.sampling_rate)
            np.save(output_file, pha_g.squeeze().cpu().numpy())
...
def main():
    ...
    parser.add_argument('--input_noisy_wavs_dir', default='../AIG/ValentiniData/train/noisy')
    parser.add_argument('--output_dir', default='../AIG/ValentiniData/train/mpsenet_phase')
    parser.add_argument('--checkpoint_file', default='cp_model/g_best')
    a = parser.parse_args()
